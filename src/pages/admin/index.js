...
export default function adminPage() {
  ...
  const [createUserData, setCreateUserData] = useState({ username: '', password: '', img: "v1681392440/d6zzeny0jiyonexeoazo.png", name: '' });
  ...
  const deleteUser = async user => {
    ...
    if (result.data.deletedCount > 0) {
      ...
      throw new Error(`Failed to delete user ${user.username}, response: ${result}`)
  }
  ...
  const handleCreateSubmit = async event => {
    ...
    if(!createUserData.username || !createUserData.password || !createUserData.img || !createUserData.name)
      return
    ...
    const response = await fetch('/api/users', {
      method: 'POST',
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ username: createUserData.username, password: createUserData.password, img: createUserData.img, name: createUserData.name })
    })
            ...
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase"><label htmlFor="password">Name</label></th>
              ...
              <tbody className="divide-y divide-gray-200">
                <tr>
                  <td className="px-6 py-4 whitespace-nowrap">
                    ...
                    onChange={ event => setCreateUserData(createUserData => ({ ...createUserData, username: event.target.value })) }></input>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input id="username" type="text" placeholder="name" 
                    onChange={ event => setCreateUserData(createUserData => ({ ...createUserData, name: event.target.value })) }></input>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <input id="password" type="text" placeholder="password"
                    onChange={ event => setCreateUserData(createUserData => ({ ...createUserData, password: event.target.value })) }></input>
                  </td>
                </tr>
                <tr>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <label className="block" htmlFor="profilePic">
                      ...
                      <CldUploadButton id="profilePic"
                        onUpload={(result, widget, _) => {
                          if (result.event === "success") {
                            setCreateUserData(createUserData => ({ ...createUserData, img: result.info.secure_url.split("upload/")[1] }));
                            widget.close();
                          }
                        }}
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase">UsrImg</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase">Following</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase">Follower</th>
              ...
              <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase">Dislikes</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-gray-200">
            {data && data.map(user => {
              return <tr key={user._id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{userImg(user)}</td>
                <td className="px-6 py-4 whitespace-nowrap">{user.following && user.following.length}</td>
                <td className="px-6 py-4 whitespace-nowrap">{user.follower && user.follower.length}</td>
                <td className="px-6 py-4 whitespace-nowrap">{user.mytweets && user.mytweets.length}</td>
                <td className="px-6 py-4 whitespace-nowrap">{user.likes && user.likes.length}</td>
                <td className="px-6 py-4 whitespace-nowrap">{user.dislikes && user.dislikes.length}</td>
              </tr>
            })}
          </tbody>
        </table>
      ...
      <div className="flex flex-col overflow-x-auto p-1.5 w-full inline-block align-middle overflow-hidden border rounded-lg">
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase">Username</th>
              <th scope="col" className="px-6 py-3 text-xs font-bold text-left text-gray-500 uppercase">User Image</th>
          ...
          <tbody className="divide-y divide-gray-200">
            {data && data.map(user => {
              return <tr key={user._id}>
                <td className="px-6 py-4 whitespace-nowrap">{user.username}</td>
                <td className="px-6 py-4 whitespace-nowrap">{userImg(user)}</td>
                <td className="px-6 py-4 whitespace-nowrap"><input type="text" placeholder={user.username}></input></td>
                <td className="px-6 py-4 whitespace-nowrap">
    ...
    <div className="xl:l-[370px] border-l border-r border-gray-200  xl:min-w-[576px] sm:ml-[100px] flex-grow px-10 pt-10">
      <div className="mx-auto inline-flex" role="group">
        <button type="button" className="px-4 py-2 text-gray-900 bg-slate-200 hover:bg-slate-300 border border-slate-300 hover:border-slate-400 mx-auto rounded-l-lg"
          onClick={() => { setDirty(true) || setMode(1) }}>
          Create User
        </button>
        <button type="button" className="px-4 py-2 text-gray-900 bg-slate-200 hover:bg-slate-300 border border-slate-300 hover:border-slate-400 mx-auto"
          onClick={() => { setDirty(true) || setMode(2) }}>
        ...
        <button type="button" className="px-4 py-2 text-gray-900 bg-slate-200 hover:bg-slate-300 border border-slate-300 hover:border-slate-400 mx-auto rounded-r-lg"
          onClick={() => { setDirty(true) || setMode(4) }}>
          Delete User
        </button>
      </div>
      ...
      {/* Table */}
      {
        !mode ? null :
          mode == 1 ? createUserTable() :
            mode == 2 ? viewUserInfoTable() :
              mode == 3 ? updateUserInfoTable() :
                mode == 4 ? deleteUserTable() : null
      }
    </div>
  );
}